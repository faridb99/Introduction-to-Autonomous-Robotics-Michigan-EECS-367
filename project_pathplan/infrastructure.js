...
function testCollision(q) {
  ...
  for (j = 0; j < range.length; j++) {
    ...
    for (i = 0; i < q.length; i++) {
      if (q[i] < range[j][i][0] || q[i] > range[j][i][1]) in_collision = false;
    }
...
function insertTreeEdge(tree, q1_idx, q2_idx) {
  ...
  // draw edge on canvas
  draw_2D_edge_configurations(
    tree.vertices[q1_idx].vertex,
    tree.vertices[q2_idx].vertex
  );
}
...
function setPlanningScene() {
  ...
  if (typeof planning_scene === "undefined") planning_scene = "empty";
...
function initSearch() {
  ...
  for (i = 1; i < url_parsed.length; i++) {
    ...
    if (
      param_parsed[0] !== "search_alg" &&
      param_parsed[0] !== "planning_scene" &&
      param_parsed[0] !== "color_scheme"
    )
      eval(param_parsed[0] + "=" + param_parsed[1]);
    else eval(param_parsed[0] + "='" + param_parsed[1] + "'");
  }
  ...
  textbar.style.left =
    25 + document.getElementById("myCanvas").offsetLeft + "px"; // position textbar wrt. canvas
